...
> ⚠️ **最新版本不再上传 exe。** 打包出来的 `THUeat.exe` 太臃肿了（单文件约 122MB，每次启动还要先解压内置文件，启动又慢又占空间），继续上传意义不大，所以从新版本起不再提供。

**想要最新版，强烈建议用上面的 `start.bat`：**
- ✅ 永远是最新版（exe 通常落后好几个版本）
- ✅ 轻量（不会塞进一个 122MB 的单文件）
- ✅ 启动快（双击秒开，不用等解压）
- ✅ 装一次 Python 就一劳永逸

> 旧版 exe 仍可在 [Releases](https://github.com/user-A100/THU-EAT/releases) 的历史版本里找到，但功能偏旧、已不再维护，不推荐使用。
